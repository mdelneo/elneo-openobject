    ...
    def _process(self,messages):
        ...
        for message in messages.filtered(lambda r:r.type.processor.id==self.id and r.state=='confirmed'):
            try:
                ...
                message.state='error'
                message.message_post( 
                            body = _("An error has occured during Landefeld EDI document processing.") + " <b><a href='#id=" + str(message.id) + "&view_type=form&model=edi.message'> " + message.name + " </a></b><br/><br/>" + unicode(e.message),
                            type = 'email',
                            subtype = "mail.mt_comment",
                            model = 'edi.message', res_id = message.id, 
                            partner_ids = [self.send_error_users])
