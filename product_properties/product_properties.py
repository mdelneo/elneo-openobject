...
class product_property_value(models.Model):
    ...
    product_property_id = fields.Many2one('product.property', 'Property', ondelete='cascade') 
    product_property_category_id = fields.Many2one('product.property.category', 'Property Category', ondelete='cascade')
    sequence = fields.Integer('Sequence')
    product_category_id = fields.Many2one('product.category', string='Product category')
    name = fields.Char('Name', size=1024, translate=True)
    default_name = fields.Char(related='product_property_id.name', string='Default Name', readonly=True)
    use_default_name = fields.Boolean("Use default name")
    used_name = fields.Char(compute='get_used', string="Used name")
    value = fields.Text('Value', translate=True)
    default_value = fields.Text(related='product_property_id.default_value', string="Default Value", readonly=True)
    use_default_value = fields.Boolean("Use default value")
    used_value = fields.Text(compute='get_used', string="Used value")
    unit_id = fields.Many2one('product.property.unit', string="Unit")
    default_unit_id = fields.Many2one(related='product_property_id.unit_id', relation='product.property.unit', string="Default Unit", readonly=True)
